#!/usr/bin/env python3
"""
NTE piano player layout helper.

Command-line MIDI / JSON piano player for the NTE three-row piano layout.

Layout matched to the in-game rows shown in the screenshot:
  TRE: Q W E R T Y U
  MID: A S D F G H J
  BAS: Z X C V B N M
...
Hotkeys:
  F5 = start playback from the beginning
  F6 = stop playback immediately
  ESC = quit program

Usage:
  python nte_piano_player.py song.mid
  python nte_piano_player.py song.json
  python nte_piano_player.py song.mid --mode key-preserve --count-in 2
  python nte_piano_player.py song.mid --export-json out_events.json

Dependencies:
  pip install mido keyboard

Optional backends:
  - If your bundle provides `异环游戏特殊键盘`, it will be used automatically.
  - Otherwise this script falls back to a simple Windows key sender via ctypes.
  - As a final fallback, it tries `pynput`.
"""
